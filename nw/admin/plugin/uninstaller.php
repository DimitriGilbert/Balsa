...
if(is_dir($path.'admin/plugin/'.$_GET['plugin']))
{
  ...
  if(is_file($p_path.'install.xml') and is_file($p_path.'installed'))
  {
	...
	//delete des fichiers de fonction
 ...
 //delete des fichiers hook
	...
	if($hk!='all')
	{
	  ...
	  foreach($hk as $h)
	  {
		unlink($path.'hook/'.$h->getAttribute('name'));
	  }
	}
	else
	{
	  $all_hk=scandir($p_path.'hook');
	  foreach($all_hk as $a_h)
	  {
		if($a_h=='.' or $a_h=='..')
		{
		 continue;
		...
		{
		  unlink($path.'hook/'.$a_h);
		}
	  }
	}
//delete des fichiers de ajax
...
//suppression des dossiers data
/*mise de coté car cela revien a supprimez toute les donnée relative aux plugins, sans gestion de dependance, ni sauvegarde :S
	...
	foreach($fold as $f)
	{
	  ...
	  {
		rmdir_r($path.'data'.$f->getAttribute('parent').'/'.$f->getAttribute('name') );
	  }
	  else
	  {
		rmdir_r($path.'data'.$f->getAttribute('name'));
	  }
	}
*/
//delete des fichiers de js
...
//flag d'installation
    ...
    hook('after_plugin_uninstall',array('plugin'=>$_GET['plugin']));
	echo 'la desinstallation de '.str_replace('_',' ',$_GET['plugin']).' c\'est bien deroule<br/><a href="'.$base_url.'admin.php">retour a l\'admin</a>';
  }
  else
  {
